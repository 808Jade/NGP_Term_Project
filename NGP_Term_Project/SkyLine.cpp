...
void main(int argc, char** argv) {
    ...
    glutDisplayFunc(drawScene);             // 화면 갱신이 필요할 때 호출
    glutKeyboardFunc(KeyBoard);             // 일반 키보드 입력
    glutKeyboardUpFunc(KeyBoardUp);         // 일반 키 뗄 때 호출
    glutSpecialFunc(SpecialKeyBoard);       // 특수 키(방향키 등) 입력 시 호출
    glutSpecialUpFunc(SpecialKeyBoardUp);   // 특수 키 뗄 때 호출
    glutMouseFunc(Mouse);                   // 마우스 버튼 클릭 시 호출
    glutMotionFunc(Motion);                 // 마우스 드래그 시 호출
    glutReshapeFunc(Reshape);               // 윈도우 크기가 변경될 때 호출
...
GLvoid BuildTimer(int value)
{
    if (h_f.game_start) {
        for (int i = 0; i < BUILDING_COUNT; ++i) {
            ...
            cout << "빌딩 생성 위치 [" << i << "]" << '\n';
            cout << "[x] : " << build[i][0].x_trans << '\n';
            cout << "[y_scale] : " << build[i][0].y_scale << '\n';
        }
        value++;
...
enum PacketType {
    PACKET_PLAYER_MOVE = 1,    // PacketPlayer
    PACKET_BUILDING_MOVE = 2,  // PacketBuildingPos
    PACKET_BULLET_MOVE = 3,    // PacketBulletPos

    PACKET_READY = 4,           // PacketReady
    PACKET_ID = 5,              // PacketID
    PACKET_SCORE = 6,           // PacketScore

    PACKET_COLLIDE_BULLET_BUILDING = 7,  // PacketCollideBB
    PACKET_COLLIDE_PLAYER_BUILDING = 8   // PacketCollidePB
};
